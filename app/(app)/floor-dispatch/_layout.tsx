...
export default function FloorDispatchLayout() {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#65435C' }}>
        ...
        screenOptions={{
          ...
          header: ({ options, navigation }) => {
            const title = typeof options.headerTitle === 'string' 
              ? options.headerTitle 
              : options.title || 'Floor Dispatch';
            return (
              <View style={{ backgroundColor: '#65435C', paddingTop: insets.top }}>
                <View className="flex-row items-center justify-between mb-1 bg-white py-3.5 px-4">
                  <TouchableOpacity
      ...
      >
        <Stack.Screen name="index" options={{ title: 'Floor Dispatch' }} />
        <Stack.Screen name="dispatch-note" options={{ title: 'Dispatch Notes' }} />
        <Stack.Screen name="dispatch-note-detail" options={{ title: 'Dispatch Note Detail' }} />
        <Stack.Screen name="initiate-dispatch" options={{ title: 'Initiate Dispatch' }} />
