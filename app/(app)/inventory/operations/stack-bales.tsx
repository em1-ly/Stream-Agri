...
const StackBalesScreen = () => {
  ...
  useEffect(() => {
    ...
    powersync.watch(
      `SELECT loc.id, loc.name, loc.warehouse_id, loc.default_location, loc.grade_id, grade.name as grade_name
       FROM warehouse_location loc
       LEFT JOIN warehouse_bale_grade grade ON loc.grade_id = grade.id
       WHERE loc.warehouse_id = ? 
       ORDER BY loc.default_location DESC, loc.name`,
        ...
        onResult: async (result) => {
          ...
          if (rows.length) {
            ...
            if (!selectedLocation || !rows.some((l) => l.id === selectedLocation.id)) {
              ...
              // Update location search text when default location is set
              if (def) {
  ...
  const getLocationDisplayName = (location: Location | null, warehouse: Warehouse | null): string => {
    ...
    let displayName = parts.length > 0 ? parts.join('/') : location.id;
    
    // Add grade in square brackets if available
    if (location.grade_name) {
      displayName += ` [${location.grade_name}]`;
    }
  ...
  const handleStartScanning = () => {
    if (!selectedWarehouse || !selectedLocation) {
      Alert.alert('Missing Info', 'Please select Warehouse and Location before starting scanning.');
      return;
    }

    router.push({
      pathname: '/(app)/inventory/operations/stack-bale-scan',
      params: {
  ...
  return (
    <>
      <Stack.Screen options={{ title: 'Stack Bales', headerShown: true }} />
      <KeyboardAvoidingView
...
export default StackBalesScreen;
