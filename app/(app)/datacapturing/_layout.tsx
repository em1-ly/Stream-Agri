  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#65435C' }}>
        ...
        screenOptions={{
          ...
          header: ({ options, navigation }) => {
            const title = typeof options.headerTitle === 'string' 
              ? options.headerTitle 
              : options.title || 'Data Capturing';
            return (
              <View style={{ backgroundColor: '#65435C', paddingTop: insets.top }}>
                <View className="flex-row items-center justify-between mb-1 bg-white py-4 px-4">
                  <TouchableOpacity
      ...
      >
        ...
        <Stack.Screen name="abitration" options={{ title: 'Abitration' }} />
      </Stack>
    </View>
  );
}
