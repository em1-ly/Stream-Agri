...
const NewSurveyResponse = () => {
    ...
    const [foundProductionCycle, setFoundProductionCycle] = useState<any>(null)
    const [searchingProductionCycle, setSearchingProductionCycle] = useState(false)
    ...
    const searchProductionCycle = async () => {
        if (!growerNumber.trim()) {
            Alert.alert('Error', 'Please enter a grower number')
            return
        }

        setSearchingProductionCycle(true)
        try {
            console.log('Searching for production cycle with grower number:', growerNumber)
            
            const productionCycle = await powersync.getAll(`
                SELECT 
                    pcr.id, 
                    pcr.grower_name, 
                    pcr.production_cycle_name, 
                    g.grower_number,
                    pcr.production_cycle_id
                FROM odoo_gms_production_cycle_registration pcr 
                JOIN odoo_gms_grower g ON pcr.grower_id = g.id 
                JOIN odoo_gms_production_cycle pc ON pcr.production_cycle_id = pc.id
                WHERE g.grower_number = ? AND pc.name = 'CY26'
            `, [growerNumber.trim()])
            
            console.log('Production cycle search result:', productionCycle)
            
            if (productionCycle.length > 0) {
                const cycle = productionCycle[0] as any
                setFoundProductionCycle(cycle)
                Alert.alert('Success', `Found production cycle for grower: ${cycle.grower_name}`)
            } else {
                setFoundProductionCycle(null)
                Alert.alert('Not Found', `No CY26 production cycle found for grower number: ${growerNumber}`)
            }
        } catch (error) {
            console.error('Error searching production cycle:', error)
            Alert.alert('Error', 'Failed to search for production cycle')
            setFoundProductionCycle(null)
        } finally {
            setSearchingProductionCycle(false)
        }
    }
    ...
    useFocusEffect (useCallback(() => {
        console.log('useEffect New Survey Screen')
    ...
    const handleSubmit = async () => {
        ...
        console.log('Found production cycle:', foundProductionCycle)

        if (!foundProductionCycle || !foundProductionCycle.id) {
            Alert.alert('Error', 'Please search and find a production cycle first')
            return
        }
    ...
    const submitSurveyUserInput = async () => {
        try {
            ...
            const result = await powersync.execute(
                ...
                [currentID, id, 'SV00002', foundProductionCycle.id, foundProductionCycle.production_cycle_id, employeeId, lat, lng, currentID]
            )
                ...
                <ScrollView className="flex-1 p-4">
                    ...
                    <View className="bg-white rounded-2xl p-4 mb-4">
                        ...
                        <View className="flex-row gap-2">
                            ...
                            />
                            <TouchableOpacity
                                className="bg-[#1AD3BB] rounded-lg p-3 px-6 items-center justify-center"
                                onPress={searchProductionCycle}
                                disabled={searchingProductionCycle}
                            >
                                <Text className="text-white font-semibold">
                                    {searchingProductionCycle ? 'Searching...' : 'Search'}
                                </Text>
                            </TouchableOpacity>
                        </View>
                        
                        {/* Display found production cycle */}
                        {foundProductionCycle && (
                            <View className="mt-3 p-3 bg-green-50 border border-green-200 rounded-lg">
                                <Text className="text-green-800 font-semibold">
                                    ✓ Found Production Cycle
                                </Text>
                                <Text className="text-green-700">
                                    Grower: {foundProductionCycle.grower_name}
                                </Text>
                                <Text className="text-green-700">
                                    Number: {foundProductionCycle.grower_number}
                                </Text>
                                <Text className="text-green-700">
                                    Cycle: {foundProductionCycle.production_cycle_name}
                                </Text>
                            </View>
                        )}
                    </View>
                    <View className="bg-white rounded-2xl p-4 mb-4">
                        ...
                        <TouchableOpacity
                            className={`rounded-xl p-4 flex-row items-center justify-center gap-2 mt-4 ${
                                foundProductionCycle ? 'bg-[#1AD3BB]' : 'bg-gray-400'
                            }`}
                            onPress={handleSubmit}
                            disabled={!foundProductionCycle}
                        >
                            <Save size={20} color="white" />
                            <Text className="text-white font-bold text-lg">
                                {foundProductionCycle ? 'Submit Survey' : 'Find Production Cycle First'}
                            </Text>
                        </TouchableOpacity>
                    </View>
                </ScrollView>
